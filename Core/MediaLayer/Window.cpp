...
Window::Window(unsigned int width, unsigned int height, const std::string& title, WindowStyle style)
{
	...
	wc.lpszClassName = "AlphaEngineWinClass";
	wc.style = CS_OWNDC;
	RegisterClass(&wc);
	
	// Configure window style
	unsigned long windowStyle = WS_CAPTION | WS_MINIMIZEBOX | WS_VISIBLE;
...
Window::~Window()
{
	...
	UnregisterClass("AlphaEngineWinClass", GetModuleHandle(NULL));
}
...
const Key Window::TranslateKey(const unsigned int code) const
{
	switch (code)
	{
