...
namespace Math
{
	...
	)
	{
		...
		genType p = Math::cross(dir, e2);

		typename genType::value_type a = Math::dot(e1, p);
		...
		genType s = orig - v0;
		baryPosition.x = f * Math::dot(s, p);
		if(baryPosition.x < typename genType::value_type(0.0f))
			return false;
		if(baryPosition.x > typename genType::value_type(1.0f))
			return false;

		genType q = Math::cross(s, e1);
		baryPosition.y = f * Math::dot(dir, q);
		if(baryPosition.y < typename genType::value_type(0.0f))
			return false;
		if(baryPosition.y + baryPosition.x > typename genType::value_type(1.0f))
			return false;

		baryPosition.z = f * Math::dot(e2, q);
