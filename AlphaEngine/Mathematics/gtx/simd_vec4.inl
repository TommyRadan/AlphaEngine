...
{
	this->Data = _mm_add_ps(this->Data , Math::detail::one);
	return *this;
}

GLM_FUNC_QUALIFIER fvec4SIMD& fvec4SIMD::operator--()
{
	this->Data = _mm_sub_ps(this->Data, Math::detail::one);
	return *this;
}
...
{
	return fvec4SIMD(_mm_add_ps(v.Data, Math::detail::one));
}

GLM_FUNC_QUALIFIER fvec4SIMD operator-- (fvec4SIMD const & v, int)
{
	return fvec4SIMD(_mm_sub_ps(v.Data, Math::detail::one));
}
...
)
{
	...
	__m128 Cmp0 = _mm_cmplt_ps(x.Data, Math::detail::zero);
	__m128 Cmp1 = _mm_cmpnlt_ps(x.Data, Math::detail::zero);
